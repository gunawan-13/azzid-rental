 ...
 * { success:true, data:{...} }
 * [...] / {...}
 * Ini mencegah tombol CRUD mati hanya karena bentuk wrapper response berbeda.
 */
async function vehicleApi(path = '', options = {}) {
  const url = VEHICLE_API_URL + path;
  ...
  if (payload && typeof payload === 'object' && payload.success === false) {
    throw new Error(payload.message || payload.error || 'Permintaan API gagal');
  }
...
function apiData(payload) {
  ...
  if (payload && typeof payload === 'object' && 'vehicle' in payload) return payload.vehicle;
  if (payload && typeof payload === 'object' && 'vehicles' in payload) return payload.vehicles;
  return payload;
}
...
function normalizeVehicleStatus(status) {
  const map = {
    ...
    rented: 'rented', Rented: 'rented', RENTED: 'rented', Booked: 'rented', booked: 'rented',
...
function normalizeVehicleFromAPI(v) {
  ...
  let feats = [];
  if (Array.isArray(v.feats)) feats = v.feats;
  else if (Array.isArray(v.features)) feats = v.features;
  else if (typeof v.features === 'string' && v.features.trim()) {
    try { feats = JSON.parse(v.features); }
    catch (_) { feats = v.features.split(',').map(x => x.trim()).filter(Boolean); }
  }
  if (!Array.isArray(feats)) feats = [];

  return {
    id: String(v.id ?? `veh-${Date.now()}-${Math.random().toString(36).slice(2,7)}`),
    name: v.name || v.nama || '',
    brand: v.brand || v.merk || '',
    model: v.model || '',
    cat: v.cat || v.type || v.category || '',
    year: Number(v.year ?? v.tahun) || '',
    plate: v.plate || v.nomor_polisi || v.license_plate || '',
    trans: v.trans || v.transmission || v.transmisi || '',
    seats: Number(v.seats ?? v.kapasitas) || 0,
    fuel: v.fuel || v.bahan_bakar || '',
    color: v.color || v.warna || '',
    doors: Number(v.doors ?? v.pintu) || 5,
    bag: v.bag || v.bagasi || '2 Koper',
    priceLK: Number(v.priceLK ?? v.price_lk ?? v.price ?? 0) || 0,
    priceDrv: Number(v.priceDrv ?? v.price_driver ?? v.price_with_driver ?? 0) || 0,
    img: v.img || v.image || v.image_url || '',
    feats,
...
const VEHICLE_BRANDS = [
  ...
  'Hyundai', 'Kia', 'Wuling', 'Mazda', 'Mercedes-Benz', 'BMW', 'Isuzu'
];

const VEHICLE_MODELS = {
  Toyota: ['Avanza', 'Innova', 'Innova Zenix', 'Fortuner', 'Alphard', 'Hiace', 'Calya', 'Veloz', 'Rush', 'Yaris'],
  Honda: ['Brio', 'Brio RS', 'HR-V', 'BR-V', 'CR-V', 'Civic', 'City'],
  Mitsubishi: ['Xpander', 'Xpander Cross', 'Pajero Sport', 'Outlander'],
  Suzuki: ['Ertiga', 'XL7', 'Jimny', 'Baleno', 'Ignis', 'APV'],
  Daihatsu: ['Xenia', 'Terios', 'Sigra', 'Ayla', 'Gran Max'],
  Nissan: ['Livina', 'Serena', 'X-Trail', 'Terra'],
  Hyundai: ['Stargazer', 'Creta', 'Santa Fe', 'Palisade', 'Staria'],
  Kia: ['Sonet', 'Seltos', 'Carnival', 'Carens'],
  Wuling: ['Almaz', 'Alvez', 'Confero', 'Cortez', 'Air ev'],
  Mazda: ['CX-5', 'CX-8', 'CX-30', 'Mazda 2'],
  'Mercedes-Benz': ['C-Class', 'E-Class', 'GLC', 'V-Class'],
  BMW: ['320i', '330i', 'X1', 'X3', 'X5'],
  Isuzu: ['Elf', 'MU-X', 'Traga']
};

const VEHICLE_COLORS = ['Black', 'White', 'Silver', 'Gray Metalik', 'Red', 'Blue', 'Brown', 'Gold'];
const VEHICLE_BAGS = ['1 Koper', '2 Koper', '3 Koper', '4 Koper', '5 Koper', '6 Koper'];
const VEHICLE_DOORS = [2, 3, 4, 5];

function vehicleBrandOptions(selected = '') {
  const values = [...new Set([...VEHICLE_BRANDS, selected].filter(Boolean))];
  return values.map(x => `<option value="${esc(x)}" ${String(selected) === String(x) ? 'selected' : ''}>${esc(x)}</option>`).join('');
}

function vehicleModelOptions(brand = 'Toyota', selected = '') {
  const base = VEHICLE_MODELS[brand] || [];
...
function syncVehicleModels() {
  const brand = $('vf_brand');
  const model = $('vf_model');
  if (!brand || !model) return;
  const selected = model.dataset.selected || model.value || '';
  model.innerHTML = vehicleModelOptions(brand.value, selected);
...
function vHome() {
  ...
  return `<section class="relative min-h-[94vh] flex items-center overflow-hidden">
  ...
  <section class="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
    <div class="flex items-end justify-between mb-8 rv">
      <div class="min-w-0"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">Armada Unggulan</span> <h2 class="font-display font-bold text-3xl mt-2">Pilihan Mobil Terbaik</h2></div>
      <a href="#/armada" class="hidden sm:flex items-center gap-2 text-sm font-semibold text-maroon-400 hover:text-red-400 transition shrink-0">Lihat Semua ${ic('arrR', 'w-4 h-4')}</a>
    </div>
  ...
  <section class="bg-ink-900 border-y border-white/5 py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
      <div class="text-center mb-14 rv"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">Cara Booking</span> <h2 class="font-display font-bold text-3xl mt-2">4 Langkah, Mobil Siap Jalan</h2></div>
      <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">${[
        ['01', 'Pilih Mobil', 'Jelajahi katalog armada dan temukan mobil yang cocok dengan kebutuhan Anda.'],
        ['02', 'Tentukan Tanggal', 'Atur tanggal mulai–selesai serta jenis rental: lepas kunci atau dengan driver.'],
        ['03', 'Login, Isi Data & Bayar', 'Masuk akun penyewa, lengkapi data, bayar via QRIS/VA/e-wallet.'],
        ['04', 'Mobil Siap Jalan', 'Konfirmasi instan, mobil diantar atau diambil di lokasi pilihan Anda.']
      ]
      .map((s, i) => `<div class="rv relative text-center px-2" style="transition-delay:${i * 100}ms">
        ...
        <h3 class="font-display font-semibold mt-4 mb-1.5">${s[1]}</h3><p class="text-[12.5px] text-muted leading-relaxed">${s[2]}</p>
      </div>`).join('')}
    </div></div>
  </section>
  <section class="max-w-7xl mx-auto px-4 sm:px-6 py-20">
  ...
  <section class="max-w-4xl mx-auto px-4 sm:px-6 pb-20">
    <div class="text-center mb-10 rv"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">FAQ</span> <h2 class="font-display font-bold text-3xl mt-2">Pertanyaan yang Sering Diajukan</h2></div>
    <div class="space-y-3">${FAQS.slice(0, 6).map((f, i) => `<div class="acc rv card overflow-hidden" style="transition-delay:${i * 60}ms">
      ...
      <div class="acc-body"><p class="px-6 pb-5 text-[13.5px] text-muted leading-relaxed">${f.a}</p></div>
    </div>`).join('')}
  </div>
  <div class="text-center mt-6"><a href="#/faq" class="text-sm font-semibold text-maroon-400 hover:text-red-400">Lihat semua FAQ →</a></div></section>
...
function filteredVeh() {
  const f = S.filters;
  let list = VEHICLES.filter(v => v.status !== 'inactive');
...
function vDetail(slug) {
  ...
  return `<section class="max-w-7xl mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="grid lg:grid-cols-[1.2fr_.8fr] gap-8">
      <div class="rv min-w-0">
        <div class="relative rounded-2xl overflow-hidden border border-white/10 h-[300px] sm:h-[440px]"><img id="galMain" src="${gal[0].s}" class="w-full h-full object-cover transition duration-500" alt="${esc(v.name)}">
          <span class="absolute top-4 left-4">${badge(v.status)}</span><span class="absolute top-4 right-4 badge bg-black/50 border-white/15 text-zinc-200">${v.cat}</span>
        </div>
      ...
      <div class="min-w-0">
        <div class="rv card p-6">
          ...
          <div class="grid grid-cols-2 gap-3 mt-5 text-[13px]">
            ${[
              ['gear', 'Transmisi', v.trans],
              ['seat', 'Kapasitas', v.seats + ' Seats'],
              ['fuel', 'Bahan Bakar', v.fuel],
              ['snow', 'AC', 'Double Blower'],
              ['door', 'Pintu', v.doors + ' Pintu'],
              ['brief', 'Bagasi', v.bag]
    ...
    <div class="mt-16"><h2 class="font-display font-bold text-2xl mb-6 rv">Mobil Serupa</h2> <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">${sim.map((x, i) => vehCard(x, i)).join('')}</div></div>
  </section>`;
}
...
function vTentang() {
  return `<section class="relative py-16 bg-ink-900 border-b border-white/5 overflow-hidden"><img src="${IMG.hero}" class="absolute inset-0 w-full h-full object-cover opacity-20"><div class="relative max-w-7xl mx-auto px-4 sm:px-6"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">Tentang Kami</span><h1 class="font-display font-extrabold text-4xl mt-2">Drive Comfort. Travel Better.</h1></div></section><section class="max-w-7xl mx-auto px-4 sm:px-6 py-16 grid lg:grid-cols-2 gap-12 items-center">
    ...
    <div class="rv min-w-0"><h2 class="font-display font-bold text-3xl mb-5">Nyaman Berkendara, <span class="text-maroon-400">Tenang Bepergian.</span></h2>
      ...
      ].map(x => `<div class="card p-5 border-maroon-500/25"><h4 class="font-display font-semibold text-maroon-400 mb-2">${x[0]}</h4><p class="text-[12.5px] text-muted leading-relaxed">${x[1]}</p></div>`).join('')}</div>
    </div>
  </section>
...
function vFaq() {
  return `<section class="relative py-16 bg-ink-900 border-b border-white/5"><div class="max-w-7xl mx-auto px-4 sm:px-6"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">FAQ</span><h1 class="font-display font-extrabold text-4xl mt-2">Butuh Jawaban Cepat?</h1></div></section><section class="max-w-4xl mx-auto px-4 sm:px-6 py-14 space-y-3">${FAQS.map((f, i) => `<div class="acc rv card overflow-hidden" style="transition-delay:${i * 50}ms"><button onclick="this.parentElement.classList.toggle('open')" class="w-full flex items-center justify-between gap-4 px-6 py-4 text-left font-semibold text-[14.5px]"><span>${f.q}</span><span class="acc-ic text-maroon-400 text-xl leading-none shrink-0">+</span></button><div class="acc-body"><p class="px-6 pb-5 text-[13.5px] text-muted leading-relaxed">${f.a}</p></div></div>`).join('')}<div class="card p-8 text-center mt-8 rv"><h3 class="font-display font-semibold text-lg mb-2">Masih ada pertanyaan?</h3><p class="text-sm text-muted mb-5">Tim kami siap membantu 24/7.</p><a href="${waLink('Halo AZZID RENTCAR, saya punya pertanyaan.')}" target="_blank" class="btn btn-m">Chat WhatsApp</a></div></section>`;
}

function contactSubmit(e){e.preventDefault();const nama=$("cf_nama_kontak").value.trim();const wa=$("cf_wa_kontak").value.trim();const email=$("cf_email_kontak").value.trim();const pesan=$("cf_pesan_kontak").value.trim();if(!nama||!wa||!pesan){toast("Lengkapi nama, WA, dan pesan","err");return}const waNumber=(S.cms&&S.cms.wa)?String(S.cms.wa).replace(/\D/g,""):"6283890354333";const text="Halo AZZID RENTCAR,%0A%0ASaya *"+encodeURIComponent(nama)+"* ingin bertanya:%0A%0A"+encodeURIComponent(pesan)+"%0A%0AKontak saya:%0AWA: "+encodeURIComponent(wa)+"%0AEmail: "+encodeURIComponent(email||"-");window.open("https://wa.me/"+waNumber+"?text="+text,"_blank");toast("Membuka WhatsApp... Silakan kirim pesan Anda.","info");e.target.reset()}
...
function vKontak() {
  return `<section class="relative py-16 bg-ink-900 border-b border-white/5"><div class="max-w-7xl mx-auto px-4 sm:px-6"><span class="text-[11px] font-bold tracking-[.3em] text-maroon-400 uppercase">Kontak</span><h1 class="font-display font-extrabold text-4xl mt-2">Hubungi Kami</h1></div></section><section class="max-w-7xl mx-auto px-4 sm:px-6 py-14 grid lg:grid-cols-2 gap-8">
    ...
    <form class="card p-7 rv" onsubmit="contactSubmit(event)">
...
function qbSubmit(e) {
  e.preventDefault();
  S.draft.start = $('qbStart').value;
  S.draft.end = $('qbEnd').value;
  S.draft.type = $('qbType').value;
  S.draft.pickup = $('qbLoc').value;
...
function isClash(vid, s, e) {
  return BOOKINGS.some(b => b.veh === vid && ['Confirmed', 'Ongoing'].includes(b.status) && s < b.end && e > b.start);
}

function bkCalc() {
  const v = veh(S.draft.veh);
  if (!v) return null;
  const dur = daysDiff(S.draft.start, S.draft.end);
  const rental = dur * v.priceLK;
  const drv = S.draft.type === 'Dengan Driver' ? dur * 150000 : 0;
  let disc = 0,
    promo = null;
  if (S.draft.promo) {
    promo = PROMOS.find(p => p.code === S.draft.promo && p.status === 'Active');
    if (promo && dur >= promo.minDays) {
      disc = promo.type === 'percent' ? Math.min(promo.cap || 1e9, rental * promo.value / 100) : promo.value;
    } else disc = 0;
  }
...
function applyPromo() {
  const c = $('promoInp').value.trim().toUpperCase();
  const dur = daysDiff(S.draft.start, S.draft.end);
...
function upsertCustomer(name, wa, email) {
  ...
  if (!c) {
    c = { id: 'CST-' + String(CUSTOMERS.length + 1).padStart(3, '0'), name, wa: wa || '', email: email || '', addr: '', total: 0, spend: 0, last: '—', status: 'New' };
    CUSTOMERS.push(c);
  }
  c.total++;
  return c;
...
function qrSVG(seed) {
  ...
  for (let y = 0; y < 21; y++)
    for (let x = 0; x < 21; x++) {
      ...
      if (inF) continue;
      if (rnd(x * 31 + y * 57 + s) > .52) cells += `<rect x="${x}" y="${y}" width="1" height="1"/>`;
    }
...
function galSwap(i, slug) {
  const v = veh(slug);
  const gal = [v.img, IMG.interior, IMG.dash, v.img];
  const m = $('galMain');
  m.style.opacity = .3;
  setTimeout(() => {
    m.src = gal[i];
    m.style.filter = i === 3 ? 'hue-rotate(-25deg) brightness(.9)' : '';
    m.style.opacity = 1;
  }, 180);
  document.querySelectorAll('.gal-th').forEach((t, j) => t.classList.toggle('!border-maroon-500', i === j));
}
...
async function authApi(path='',options={}){
  ...
  if(payload?.success===false)throw new Error(payload.message||'Permintaan autentikasi gagal');
  return payload;
}
...
async function doGoogleLogin(credential){
  try{
    const result=await authApi('/google',{method:'POST',body:JSON.stringify({credential})});
    const user=apiData(result)?.user||result?.user;
    if(!user||user.role!=='user') throw new Error('Login Google hanya tersedia untuk akun penyewa.');
    S.custSession={id:user.id,email:user.email,nama:user.name,phone:user.phone||''}; closeModal(); toast('Login Google berhasil. Selamat datang, '+user.name.split(' ')[0]+'!'); renderC();
  }catch(err){ const box=$('mErrIn')||$('bkAuthErr'); if(box){box.textContent=err.message||'Login Google gagal.';box.classList.remove('hidden');} else toast(err.message||'Login Google gagal.','err'); }
}
function openForgotPassword(){
  modal(`<div class="p-7"><div class="flex justify-between items-start mb-5"><div><h3 class="font-display font-bold text-xl">Lupa Password</h3><p class="text-[12px] text-muted mt-1">Masukkan email akun Anda. Kami akan mengirim link untuk membuat password baru.</p></div><button onclick="closeModal()" class="text-muted">${ic('x')}</button></div><div id="forgotErr" class="hidden mb-3 rounded-lg border border-red-500/40 bg-red-500/10 text-red-300 text-[12px] px-3 py-2"></div><label class="lbl">Email</label><input id="forgotEmail" class="inp mb-4" type="email" autocomplete="email" placeholder="email@anda.com" onkeydown="if(event.key==='Enter')doForgotPassword()"><button onclick="doForgotPassword()" class="btn btn-m w-full">Kirim Link Reset</button><button onclick="openAuth('in')" class="btn btn-g w-full mt-3">Kembali ke Login</button></div>`);
}
async function doForgotPassword(){
  const email=($('forgotEmail').value||'').trim().toLowerCase(), err=$('forgotErr'); err.classList.add('hidden');
  if(!email){err.textContent='Email wajib diisi.';err.classList.remove('hidden');return;}
  try{await authApi('/forgot-password',{method:'POST',body:JSON.stringify({email})}); modal(`<div class="p-7 text-center"><div class="w-12 h-12 rounded-full bg-emerald-500/10 text-emerald-300 grid place-items-center mx-auto mb-4">✓</div><h3 class="font-display font-bold text-xl mb-2">Cek email Anda</h3><p class="text-sm text-muted leading-relaxed">Jika email terdaftar, link reset password sudah dikirim. Link berlaku 30 menit.</p><button onclick="openAuth('in')" class="btn btn-m w-full mt-5">Kembali ke Login</button></div>`);}
  catch(e){err.textContent=e.message||'Gagal mengirim link reset.';err.classList.remove('hidden');}
}
function openResetPassword(token){
  modal(`<div class="p-7"><div class="flex justify-between items-start mb-5"><div><h3 class="font-display font-bold text-xl">Buat Password Baru</h3><p class="text-[12px] text-muted mt-1">Masukkan password baru minimal 6 karakter.</p></div><button onclick="closeModal()" class="text-muted">${ic('x')}</button></div><div id="resetErr" class="hidden mb-3 rounded-lg border border-red-500/40 bg-red-500/10 text-red-300 text-[12px] px-3 py-2"></div><label class="lbl">Password Baru</label><input id="resetPass" type="password" class="inp mb-4" placeholder="Minimal 6 karakter"><button onclick="doResetPassword(${JSON.stringify(token)})" class="btn btn-m w-full">Simpan Password Baru</button></div>`);
}
async function doResetPassword(token){
  const p=$('resetPass').value, err=$('resetErr'); err.classList.add('hidden'); if(p.length<6){err.textContent='Password minimal 6 karakter.';err.classList.remove('hidden');return;}
  try{await authApi('/reset-password',{method:'POST',body:JSON.stringify({token,password:p})}); history.replaceState({},document.title,location.pathname+location.hash); modal(`<div class="p-7 text-center"><div class="w-12 h-12 rounded-full bg-emerald-500/10 text-emerald-300 grid place-items-center mx-auto mb-4">✓</div><h3 class="font-display font-bold text-xl mb-2">Password berhasil diubah</h3><p class="text-sm text-muted">Silakan login menggunakan password baru Anda.</p><button onclick="openAuth('in')" class="btn btn-m w-full mt-5">Login Sekarang</button></div>`);}
  catch(e){err.textContent=e.message||'Link reset tidak valid atau sudah kedaluwarsa.';err.classList.remove('hidden');}
}
...
async function doLogin(){const e=($('lgE').value||'').trim().toLowerCase(),p=$('lgP').value,box=$('loginCard'),errBox=$('lgErr');errBox.classList.add('hidden');try{const result=await authApi('/login',{method:'POST',body:JSON.stringify({email:e,password:p})});const user=apiData(result)?.user||result?.user;if(!user||user.role!=='admin')throw new Error('Akun ini bukan akun admin.');S.session={id:user.id,name:user.name,email:user.email,role:user.role};localStorage.setItem('azzid_has_session','1');S.adminView='overview';renderA();syncAdminBtns();loadAdminUsers();toast('Selamat datang, '+user.name+' — Dashboard Admin aktif.')}catch(error){box.classList.remove('shake');void box.offsetWidth;box.classList.add('shake');errBox.textContent=error.message||'Login gagal. Periksa email dan password.';errBox.classList.remove('hidden')}}
async function adminLogout(){try{await authApi('/logout',{method:'POST'})}catch(_){}S.session=null;localStorage.removeItem('azzid_has_session');renderA();syncAdminBtns();toast('Anda telah logout','info')}
async function restoreAuth(){if(!localStorage.getItem('azzid_has_session')){S.session=null;S.custSession=null;syncAdminBtns();return}try{const result=await authApi('/me');const user=apiData(result);if(user?.role==='admin'){S.session={id:user.id,name:user.name,email:user.email,role:user.role};loadAdminUsers()}else if(user?.role==='user'){S.custSession={id:user.id,email:user.email,nama:user.name,phone:user.phone||''}}syncAdminBtns();if(location.hash.startsWith('#/admin'))renderA()}catch(_){S.session=null;S.custSession=null;localStorage.removeItem('azzid_has_session');syncAdminBtns();}}
    ...
    app.innerHTML = `<div class="min-h-screen grid place-items-center relative overflow-hidden bg-ink-950 px-4 py-10">
      ...
      <div class="relative w-full max-w-4xl grid lg:grid-cols-2 card overflow-hidden bg-ink-800/90 backdrop-blur-xl rv on" id="loginCard">
        ...
        <div class="p-7 sm:p-9">
          ...
          <button onclick="doLogin()" class="btn btn-g w-full">${ic('lock', 'w-4 h-4')} Masuk Dashboard</button><p class="text-[11px] text-zinc-500 text-center mt-4">Gunakan akun admin.</p><div class="flex mt-6 text-[12px]"><a href="#/" class="text-muted hover:text-white">← Kembali ke Website</a></div>
        </div>
      </div>
    </div>`;
    return;
...
function areaChart(data, h = 210) {
  ...
  let line = '',
    area = `M0 ${h-14} `;
  data.forEach((p, i) => {
    line += (i ? 'L' : 'M') + px(i).toFixed(1) + ' ' + py(p.v).toFixed(1) + ' ';
    area += `L${px(i).toFixed(1)} ${py(p.v).toFixed(1)} `;
  });
  area += `L${W} ${h-14} Z`;
  ...
  return `<svg viewBox="0 0 ${W} ${h}" class="w-full"><defs><linearGradient id="gv" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#991B1B" stop-opacity=".45"/><stop offset="1" stop-color="#991B1B" stop-opacity="0"/></linearGradient></defs>
    ...
    <path d="${area}" fill="url(#gv)"/><path d="${line}" fill="none" stroke="#c53030" stroke-width="2.5" class="chart-line"/>${dots}
  </svg>`;
}

async function updateUserRole(id,role){try{const response=await fetch(`${USERS_API_URL}/${id}/role-status`,{credentials:'include',method:'PATCH',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({role})});const result=await response.json();if(!response.ok||result.success===false)throw new Error(result.message||`HTTP ${response.status}`);const user=apiData(result);const i=ADMIN_USERS.findIndex(x=>String(x.id)===String(id));if(i>=0&&user)ADMIN_USERS[i]=user;toast('Role user berhasil diperbarui');renderAdminBody()}catch(err){toast(err.message||'Gagal memperbarui role','err');renderAdminBody()}}
    ...
    <div class="grid xl:grid-cols-[1.6fr_1fr] gap-6">
      <div class="rv card p-6 min-w-0"><div class="flex flex-wrap items-center justify-between gap-3 mb-5"><h3 class="font-display font-semibold">Revenue Analytics</h3>
        <div class="flex gap-1.5">${[
          ...
          ['30', '30 Hari'],
          ['90', '3 Bulan']
        ].map(r => `<button onclick="S.revRange=${r[0]};renderAdminBody()" class="chip !py-1.5 text-[11px] ${S.revRange == +r[0] ? 'on' : ''}">${r[1]}</button>`).join('')}</div></div>
        ${areaChart(series)}
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mt-5 pt-5 border-t border-white/5 text-center">
          ${[
            ['Total Revenue', fmtK(tot)],
            ['Paid Booking', BOOKINGS.filter(b => b.pay.s === 'PAID').length],
            ['Pending', BOOKINGS.filter(b => b.pay.s === 'PENDING').length],
            ['Refund', BOOKINGS.filter(b => b.pay.s === 'REFUNDED').length],
            ['Avg Booking', fmtK(tot / 22)]
...
function aRental() {
  ...
  return `<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
      ${[
        ['wheel', 'SEWA BERJALAN', ongoing.length, 'text-orange-300'],
...
function extendRental(id) {
  ...
  b.end = addDays(b.end, 1);
  b.total += per;
  if (b.type === 'Dengan Driver') { b.sub += v ? v.priceLK : 350000;
    b.drv += 150000; } else b.sub += per;
...
/* ================= BOOKING CRUD ================= */
function calcBf() {
  const v = veh($('bf_veh').value);
  const s = $('bf_start').value,
    e = $('bf_end').value;
  const el = $('bfTotal');
  if (!el) return;
  if (!v || !s || !e || e < s) { el.textContent = '—'; return; }
  const dur = daysDiff(s, e);
  el.textContent = fmtIDR(dur * v.priceLK + ($('bf_type').value === 'Dengan Driver' ? dur * 150000 : 0));
}
...
function bookingForm(id) {
  ...
  modal(`<div class="p-7"><div class="flex justify-between items-center mb-5 gap-3"><h3 class="font-display font-semibold text-lg">${b ? 'Edit Booking' : 'Buat Booking Manual'}</h3><button onclick="closeModal()" class="text-muted hover:text-white shrink-0">${ic('x')}</button></div>
    ...
    <button onclick="saveBooking('${id || ''}')" class="btn btn-m w-full mt-6">${ic('check', 'w-4 h-4')} Simpan Booking</button>
  </div>`, 1);
  calcBf();
}
...
function delBooking(id) {
  ...
  modal(`<div class="p-7 text-center"><span class="w-14 h-14 mx-auto rounded-full bg-red-500/15 border border-red-500/40 text-red-300 grid place-items-center mb-4">${ic('alert')}</span>
    ...
    <div class="grid grid-cols-2 gap-2.5 mt-6"><button onclick="closeModal()" class="btn btn-g btn-sm">Batal</button> <button onclick="hardDelBooking('${id}')" class="btn btn-d btn-sm">${ic('trash', 'w-4 h-4')} Hapus</button></div>
  </div>`);
}
...
function refreshBk() {
  ...
  const cnt = $('bkCnt');
  if (cnt) cnt.textContent = list.length + ' booking';
}
...
function openBookingDetail(id) {
  ...
  const cu = CUSTOMERS.find(c => c.name === b.cust) || {};
  modal(`<div class="p-7">
...
function aCalendar() {
  ...
  const rows = VEHICLES.filter(v => v.status !== 'inactive').map(v => {
    const bars = BOOKINGS.filter(b => b.veh === v.id && ['Confirmed', 'Ongoing', 'Completed'].includes(b.status)).map(b => {
      const s = dP(b.start),
        e = dP(b.end);
      if (s > new Date(y, m + 1, 0) || e < new Date(y, m, 1)) return '';
      const sD = s.getFullYear() === y && s.getMonth() === m ? s.getDate() : 1;
      const eD = e.getFullYear() === y && e.getMonth() === m ? e.getDate() : dim;
      return `<button onclick="openBookingDetail('${b.id}')" class="gantt-bar absolute top-1/2 -translate-y-1/2 h-7 rounded-md bg-gradient-to-r from-maroon-600 to-maroon-500 border border-maroon-400/40 flex items-center px-1.5 text-[9px] font-bold truncate" style="left:${(sD - 1) * 36 + 2}px;width:${Math.max(1, eD - sD + 1) * 36 - 4}px" title="${b.id} · ${b.cust} · ${dShort(b.start)}–${dShort(b.end)}">${b.cust.split(' ')[0].toUpperCase()}</button>`;
    }).join('');
...
function vehStats() {
  return {
    ...
    av: VEHICLES.filter(v => v.status === 'available').length,
    rt: VEHICLES.filter(v => v.status === 'rented').length,
    mt: VEHICLES.filter(v => v.status === 'maintenance').length
  };
}
    ...
    <div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
      ${[
        ...
        <div class="min-w-0">
          ...
          <div class="text-[10px] uppercase tracking-widest text-muted truncate">${x[1]}</div>
        </div>
      </div>`).join('')}
    </div>
    <div class="rv card p-4 flex flex-wrap gap-3 items-center">
      ...
      <select class="inp !w-40" onchange="S._vst=this.value;refreshVeh()">
        <option value="">Semua Status</option>
        ${['available', 'rented', 'maintenance', 'inactive'].map(s => `<option ${S._vst === s ? 'selected' : ''} value="${s}">${s}</option>`).join('')}
      </select>
      <select class="inp !w-40" onchange="S._vc=this.value;refreshVeh()">
        <option value="">Semua Kategori</option>
        ${['City Car', 'MPV', 'SUV', 'Premium', 'Commercial'].map(s => `<option ${S._vc === s ? 'selected' : ''}>${s}</option>`).join('')}
      </select>
    ...
    <div class="rv card p-5">
      ...
      <div class="space-y-2 max-h-56 overflow-y-auto">
        ${LOGS.slice(0, 12).map(l => `<div class="flex gap-3 text-[12.5px] bg-ink-900 rounded-lg px-4 py-2.5">
          <span class="text-muted shrink-0 font-mono text-[11px]">${l.t}</span>
          <span class="min-w-0">${esc(l.txt)}</span>
        </div>`).join('') || '<p class="text-muted text-[13px]">Belum ada aktivitas.</p>'}
      </div>
    </div>
  </div>`;
}
...
function vehRowHtml(v) {
  const cnt = BOOKINGS.filter(b => b.veh === v.id).length;
  console.log('🚗 Render vehicle:', v);
  return `<tr>
    <td>
      <div class="flex items-center gap-3 min-w-0">
        <img src="${v.img}" class="w-12 h-9 rounded-lg object-cover shrink-0">
        <div class="min-w-0">
          <div class="font-semibold truncate">${esc(v.name)}</div>
          <div class="text-[10.5px] text-muted truncate">${v.brand} ${esc(v.model || '')} · ${v.trans} · ${v.seats} seats</div>
        </div>
      </div>
    </td>
    <td class="font-mono text-[12px] whitespace-nowrap">${v.plate || '-'}</td>
    <td><span class="inline-block w-3 h-3 rounded-full mr-1" style="background-color:${v.color || '#000'}"></span>${v.color || '-'}</td>
    <td>${v.cat || '-'}</td>
    <td>${v.year || '-'}</td>
    <td class="whitespace-nowrap">${fmtK(v.priceLK)}</td>
    <td class="whitespace-nowrap">${fmtK(v.priceDrv || v.priceLK + 150000)}</td>
    <td><span class="text-[12px] text-muted">${cnt}×</span></td>
    <td>
      <select class="inp !w-[130px] !py-1.5 text-[11px]" onchange="setVehStatus('${v.id}',this.value)">
        ${['available', 'rented', 'maintenance', 'inactive'].map(s => `<option value="${s}" ${v.status === s ? 'selected' : ''}>${s}</option>`).join('')}
      </select>
    </td>
    <td>
...
function refreshVeh() {
  ...
  if (!tb) return;
  let list = [...VEHICLES];
  const q = S._vq ? S._vq.toLowerCase() : '';
...
function vehForm(id) {
  ...
  const model = v ? v.model : 'Avanza';
  const modelOpts = vehicleModelOptions(brand, model);

  modal(`<div class="p-7">
    <div class="flex justify-between items-center mb-5 gap-3">
      <h3 class="font-display font-semibold text-lg">${v ? 'Edit Aset' : 'Tambah Aset Baru'}</h3>
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      ...
      <div><label class="lbl">Brand</label><select id="vf_brand" class="inp" onchange="syncVehicleModels()">${vehicleBrandOptions(brand)}</select></div>
      <div><label class="lbl">Model</label><select id="vf_model" class="inp" data-selected="${esc(model)}">${modelOpts}</select></div>
      <div><label class="lbl">Tahun</label><input id="vf_tahun" type="number" min="1990" max="2100" class="inp" value="${v ? v.year : 2024}"></div>
      ${f('plate', v ? v.plate : 'B 1000 AZD', 'Nomor Polisi')}
      <div><label class="lbl">Transmisi</label><select id="vf_trans" class="inp">
        ${['Automatic', 'Manual', 'CVT', 'DCT'].map(x => `<option ${v && v.trans === x ? 'selected' : ''}>${x}</option>`).join('')}
      </select></div>
      <div><label class="lbl">Kapasitas (seats)</label><input id="vf_seats" type="number" min="1" class="inp" value="${v ? v.seats : 7}"></div>
      <div><label class="lbl">Warna</label><select id="vf_warna" class="inp">${[...new Set([...VEHICLE_COLORS, v?.color].filter(Boolean))].map(x => `<option ${v && v.color === x ? 'selected' : ''}>${esc(x)}</option>`).join('')}</select></div>
      <div><label class="lbl">Bahan Bakar</label><select id="vf_fuel" class="inp">
        ${['Bensin', 'Diesel', 'Hybrid', 'Listrik'].map(x => `<option ${v && v.fuel === x ? 'selected' : ''}>${x}</option>`).join('')}
      </select></div>
      <div><label class="lbl">Kategori</label><select id="vf_cat" class="inp">
        ${['City Car', 'MPV', 'SUV', 'Premium', 'Commercial'].map(x => `<option ${v && v.cat === x ? 'selected' : ''}>${x}</option>`).join('')}
      </select></div>
      <div><label class="lbl">Jumlah Pintu</label><select id="vf_doors" class="inp">
        ${VEHICLE_DOORS.map(x => `<option value="${x}" ${Number(v?.doors || 5) === x ? 'selected' : ''}>${x} Pintu</option>`).join('')}
      </select></div>
      <div><label class="lbl">Bagasi</label><select id="vf_bag" class="inp">
        ${[...new Set([...VEHICLE_BAGS, v?.bag].filter(Boolean))].map(x => `<option ${v && v.bag === x ? 'selected' : ''}>${esc(x)}</option>`).join('')}
      </select></div>
      <div><label class="lbl">Harga Lepas Kunci (Rp/hari) *</label><input id="vf_harga" type="number" min="0" class="inp" value="${v ? v.priceLK : 400000}"></div>
      ...
      <div class="sm:col-span-2"><label class="lbl">Fasilitas</label>
        <div class="flex flex-wrap gap-2">${FAC_OPTS.map(x => `<label class="chip cursor-pointer ${v && v.feats && v.feats.includes(x) ? 'on' : ''}">
          <input type="checkbox" class="hidden fac-chk" value="${esc(x)}" ${v && v.feats && v.feats.includes(x) ? 'checked' : ''} onchange="this.parentElement.classList.toggle('on')">${esc(x)}</label>`).join('')}</div>
      </div>
      ...
      <div><label class="lbl">Status</label><select id="vf_status" class="inp">
        ${['available', 'rented', 'maintenance', 'inactive'].map(s => `<option value="${s}" ${v && v.status === s ? 'selected' : ''}>${s}</option>`).join('')}
      </select></div>
    </div>
    <button onclick="saveVeh('${id || ''}')" class="btn btn-m w-full mt-6">${ic('check', 'w-4 h-4')} Simpan Aset</button>
  </div>`, 1);
}
...
function custDetail(id) {
  ...
  const hist = BOOKINGS.filter(b => b.cust === c.name);
  ...
  <div class="grid grid-cols-3 gap-3 mb-6 text-center">${[
      ['Total Booking', c.total],
      ['Total Spending', fmtK(c.spend)],
      ['Last Rental', esc(c.last)]
    ].map(x => `<div class="card !bg-ink-900 p-4 min-w-0"><div class="font-display font-bold text-maroon-400 truncate">${x[1]}</div><div class="text-[10px] uppercase tracking-wider text-muted mt-1">${x[0]}</div></div>`).join('')}</div>
  ...
  <a href="${waLink('Halo ' + c.name + ', terima kasih telah menjadi pelanggan AZZID RENTCAR.')}" target="_blank" class="btn btn-m btn-sm w-full mt-4">Hubungi via WhatsApp</a>
  </div>`, 1);
}
    ...
    <div class="rv card p-6 overflow-x-auto"><h3 class="font-display font-semibold mb-4">Role Permission Matrix</h3>
      <table class="tbl min-w-[680px]"><thead><tr><th>Modul</th><th>Super Admin</th><th>Admin</th><th>Staff</th><th>Finance</th><th>Driver</th></tr></thead><tbody>
        ${[
          ...
          ['Reports', '✓', '—', '—', '✓', '—'],
          ['CMS & Settings', '✓', '—', '—', '—', '—']
        ].map(r => `<tr><td class="font-semibold whitespace-nowrap">${r[0]}</td>${r.slice(1).map(c => `<td class="${c === '✓' ? 'text-emerald-300' : c === '◐' ? 'text-amber-300' : 'text-zinc-600'} text-lg">${c}</td>`).join('')}</tr>`).join('')}
      </tbody></table>
    </div>
  </div>`;
}
...
function openInvoice(id) {
  ...
  $('printSheet').innerHTML = `<div style="font-family:Arial,sans-serif;color:#111;max-width:700px;margin:0 auto;padding:32px">
    ...
    <table style="width:100%;margin-top:24px;border-collapse:collapse;font-size:13px">
      ...
      <tr><td style="padding:10px 8px;font-weight:800;font-size:15px">TOTAL</td><td style="text-align:right;padding:10px 8px;font-weight:800;font-size:15px;color:#7F1D1D">${fmtIDR(b.total)}</td></tr>
    </table>
    ...
    <p style="margin-top:28px;font-size:11px;color:#555">Terima kasih telah percaya pada AZZID RENTCAR — Nyaman Berkendara, Tenang Bepergian.</p>
  </div>`;
  window.print();
}
...
function renderC() {
  ...
  const p = h.split('/');
  const main = $('cMain');
  $('custApp').classList.remove('hidden');
