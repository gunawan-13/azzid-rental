  ...
  app.innerHTML = `<div class="min-h-screen bg-ink-950 flex">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <header class="h-16 bg-ink-900/80 backdrop-blur border-b border-white/5 flex items-center gap-3 px-4 sm:px-6 sticky top-0 z-30">
        ...
        <div class="relative ml-auto"><button onclick="document.getElementById('bellD').classList.toggle('hidden')" class="p-2 relative text-zinc-400 hover:text-white">${ic('bell')}<span id="adminNotifBadge" class="hidden absolute top-1 right-1 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[16px] h-4 px-1 grid place-items-center">0</span></button>
          <div id="bellD" class="hidden absolute right-0 top-12 w-[300px] max-w-[88vw] card bg-ink-800 p-2 z-50 shadow-card"><div class="px-3 py-2 text-[12px] font-bold text-muted uppercase tracking-wider">Notifikasi</div>${(typeof buildNotifs === "function" ? (ADMIN_NOTIFS && ADMIN_NOTIFS.length ? ADMIN_NOTIFS : NOTIFS)).map(n => `<div class="flex gap-3 px-3 py-2.5 rounded-lg hover:bg-white/5"><span class="${n.cl} mt-0.5 shrink-0">${ic(n.ic, 'w-4 h-4')}</span><div class="min-w-0"><p class="text-[12.5px] leading-snug">${n.t}</p><p class="text-[10.5px] text-muted mt-0.5">${n.w}</p></div></div>`).join('')}</div></div>
