...
function addUserNotif(email, type, message){
  ...
  USER_NOTIFS.unshift({
    ...
    w: new Date().toLocaleString('id-ID', {day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}), dibaca: false
  });
...
function aOverview() {
  ...
  return `<div class="space-y-6">
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <div class="rv card p-5 hover:border-maroon-500/40 transition"><div class="flex items-center justify-between mb-3"><span class="w-10 h-10 rounded-xl bg-maroon-500/15 text-maroon-400 grid place-items-center">${ic('file')}</span></div><div class="font-display font-extrabold text-[22px] leading-none"><span data-cu=String(BOOKINGS.length)>0</span></div><div class="text-[11px] uppercase tracking-widest text-muted mt-2">Total Booking</div><div class="text-[11px] text-emerald-300 mt-1">↑ 12% vs bulan lalu</div></div>
      <div class="rv card p-5 hover:border-maroon-500/40 transition" style="transition-delay:80ms"><div class="flex items-center justify-between mb-3"><span class="w-10 h-10 rounded-xl bg-maroon-500/15 text-maroon-400 grid place-items-center">${ic('clock')}</span></div><div class="font-display font-extrabold text-[22px] leading-none"><span data-cu=String(BOOKINGS.filter(b => ['Ongoing','Confirmed'].includes(b.status)).length)>0</span></div><div class="text-[11px] uppercase tracking-widest text-muted mt-2">Booking Aktif</div><div class="text-[11px] text-emerald-300 mt-1">↑ 8% vs bulan lalu</div></div>
