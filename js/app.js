...
async function custLogout(){try{await authApi('/logout',{method:'POST'})}catch(_){}S.custSession=null;closeModal();toast('Anda telah keluar dari akun','info');renderC()}
    ...
    app.innerHTML = `<div class="min-h-screen grid place-items-center relative overflow-hidden bg-ink-950 px-4 py-10">
      ...
      <div class="relative w-full max-w-4xl grid lg:grid-cols-2 card overflow-hidden bg-ink-800/90 backdrop-blur-xl rv on" id="loginCard">
        ...
        <div class="p-7 sm:p-9">
          ...
          <button onclick="doLogin()" class="btn btn-g w-full">${ic('lock', 'w-4 h-4')} Masuk Dashboard</button>< class="text-[11px] text-zinc-500 text-center mt-4">Gunakan akun admin yang terdaftar di database MySQL.<div class="flex mt-6 text-[12px]"><a href="#/" class="text-muted hover:text-white">← Kembali ke Website</a></div>
        </div>
      </div>
    </div>`;
    return;
